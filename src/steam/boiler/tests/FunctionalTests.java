...
public class FunctionalTests {
  ...
  private void test_normal_operation(int time, int numberOfPumps) {
    ...
    if (model.getBoiler().getWaterLevel() > config.getMaximalLimitLevel()) {
      fail("Water level above limit maximum (after " + time + "s with "  //$NON-NLS-1$ //$NON-NLS-2$
          +  numberOfPumps
          + " pumps)"); //$NON-NLS-1$
    }
    if (model.getBoiler().getWaterLevel() < config.getMinimalLimitLevel()) {
      fail("Water level below limit minimum (after " + time  //$NON-NLS-1$
          + "s with " + numberOfPumps //$NON-NLS-1$ 
          + " pumps)"); //$NON-NLS-1$
    }
  }
 ...
 */
  private void test_degraded_operation(MySteamBoilerController controller,
    ...
    if (model.getBoiler().getWaterLevel() > config.getMaximalLimitLevel()) {
      fail("Water level above limit maximum (after " + time  //$NON-NLS-1$
          +  "s with " + config.getNumberOfPumps() //$NON-NLS-1$
          + " pumps)"); //$NON-NLS-1$
    }
    if (model.getBoiler().getWaterLevel() < config.getMinimalLimitLevel()) {
      fail("Water level below limit minimum (after " + time  //$NON-NLS-1$
